...
export function calculateNextOccurrence(transaction: RecurringTransaction) {
    ...
    switch (transaction.recurrenceUnit) {
        ...
        case "MONTH":
            if(!transaction.dayOfMonth) throw new AppError("API_ERROR", "dayOfMonth is required for MONTH recurrence")
            return addMonths(baseDate, transaction.recurrenceValue, transaction.dayOfMonth)
        default:
