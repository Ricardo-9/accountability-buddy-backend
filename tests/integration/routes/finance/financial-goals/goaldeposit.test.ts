...
describe("POST /goals/deposit", () => {
    ...
    describe("Happy path", () => {
        it("should return 200, goal deposit and new balance", async () => {
            ...
            vi.mocked(adjustBalanceWithTx).mockResolvedValue({ balance: new Prisma.Decimal(2000) } as any)
    ...
    describe("API validations", () => {
        ...
        it("should throw 500 if internal errors occurs", async () => {
            ...
            mockTx.goalProgressSnapshot.findFirst.mockResolvedValue({ totalDeposited: new Prisma.Decimal(100) })
            vi.mocked(adjustBalanceWithTx).mockResolvedValue({ balance: new Prisma.Decimal(2000) } as any)
            mockTx.goalProgressSnapshot.create.mockRejectedValue(new Error("Db error"))
    ...
    describe("Zod validations", () => {
        ...
        it("should thrrow 400 if amount is invalid", async () => {
            ...
            expect(response.status).toBe(400)
            expect(response.body.error.details[0].message).toBe("Invalid amount value")
        })
        ...
        it("should throw 400 if amount <= 0", async () => {
            ...
            expect(response.status).toBe(400)
            expect(response.body.error.details[0].message).toBe("Amount must be greater than $0")
        })
