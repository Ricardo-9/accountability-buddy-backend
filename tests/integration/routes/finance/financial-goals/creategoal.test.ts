...
import { Prisma, DurationUnit, InvestorStyle, FinanceAccount } from "@prisma/client";
...
describe("POST /goals", () => {
    ...
    describe("Happy path", () => {
        beforeEach(() => vi.mocked(txMock.financeAccount.findUnique).mockResolvedValue({ userId: "userId" }))

        it("should return 201, goal data and new balance when categoryId is not null and valid", async () => {
            txMock.financialCategory.findFirst.mockResolvedValue({ id: "eba6db67-31b1-4e29-aeb9-bfc8fdf40fca" })
        ...
        it("should return 201, goal data and new balance when categoryId null", async () => {
            ...
            expect(response.body).toMatchObject({
                data: {
                    goal: { ...mockGoal, target: "1000", initialAmount: "100", categoryId: null },
                    newBalance: "100"
                }
            })
        })
        ...
        it("should return 201, goal data and new balance when categoryId is not provided", async () => {
            ...
            expect(txMock.financialGoal.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("API errors", () => {
        ...
        it("should throw 422 (INSUFFICIENT_FUNDS) when balance < initialAmount", async () => {
            txMock.financeAccount.findUnique.mockResolvedValue({ userId: "userId" })
            adjustBalanceWithTxMock.mockRejectedValue(new AppError("INSUFFICIENT_FUNDS", "Insufficient balance", 422))
    ...
    describe("Validation errors", () => {
        describe("Missing fields", () => {
            ...
            it("should throw 400 if target is not provided", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Target is required")
            })
            ...
            it("should throw 400 if initialAmount is not provided", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Initial amount is required")
            })
            ...
            it("should throw 400 if durationValue is not provided", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Duration value is required")
            })
            ...
            it("should throw 400 if durationUnit is not provided", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid duration unit")
            })
            ...
            it("should throw 400 if style is not provided", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid investor style")
            })
        })
        describe("Type errors", () => {
            ...
            it("should throw 400 if target is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Target must be a number")
            })
            ...
            it("should throw 400 if initialAmount is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Initial amount must be a number")
            })
            ...
            it("should throw 400 if durationValue is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Duration value must be a number")
            })
            ...
            it("should throw 400 if duration value is not a integer", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Duration must be an integer")
            })
        ...
        describe("Value errors", () => {
            it("should throw 400 if target <= 0", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Target value must be greater than $0")
            })
            ...
            it("should throw 400 if initialAmount is negative", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Initial amount cannot be negative")
            })
            ...
            it("should throw 400 if durationValue < 1", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Duration must be at least 1")
            })
        })
        describe("API validations", () => {
            it("should throw 400 if initialAmount > target", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Initial amount cannot be greater than target value")
            })
