...
vi.mock("jose", async (importOriginal) => {
    ...
    return {
        ...
        jwtVerify: vi.fn().mockResolvedValue({
            payload: { sub: "83793157-f162-490c-b503-ea5983ab04b7", email: "user@test.com" }
        })
    }
})
...
describe("PATCH /goals", () => {
    ...
    describe("Happy path", () => {
        it("should return 200 and updated goal", async () => {
            ...
            mockTx.financialGoal.update.mockResolvedValue({ ...mockGoal, name: "New name" })
        ...
        it("should decrement balance when initial amount increases", async () => {
            ...
            mockTx.financialGoal.update.mockResolvedValue({ ...mockGoal, initialAmount: new Prisma.Decimal(100) })
        ...
        it("should increment balance when initial amount decreases", async () => {
            ...
            mockTx.financialGoal.update.mockResolvedValue({ ...mockGoal, initialAmount: new Prisma.Decimal(9) })
        ...
        it("should return new balance if balance was changed", async () => {
            ...
            vi.mocked(adjustBalanceWithTx).mockResolvedValue({ balance: new Prisma.Decimal(100) } as any)
            mockTx.financialGoal.update.mockResolvedValue(mockGoal)
    ...
    describe("Zod validations", () => {
        describe("Type errors", () => {
            ...
            it("should throw 400 if name is not a string", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid goal name")
            })
            ...
            it("should throw 400 if target is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid target value")
            })
            ...
            it("should throw 400 if initialAmount is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid initial amount value")
            })
            ...
            it("should throw 400 if durationValue is not a number", async () => {
                ...
                expect(response.status).toBe(400)
                expect(response.body.error.details[0].message).toBe("Invalid duration value")
            })
