...
describe("DELETE /transactions/:id", () => {
    ...
    authMiddlewareTests("delete", `/finance/transactions/${transactionId}`, "FINANCES")

    it("should return 200 and delete the recurring transaction", async () => {
        vi.mocked(prisma.recurringTransaction.updateMany).mockResolvedValue({ count: 1 })
    ...
    it("should throw 500 if db fails", async () => {
        vi.mocked(prisma.recurringTransaction.updateMany).mockRejectedValue(new Error("Db error"))
