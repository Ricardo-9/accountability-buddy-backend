...
import { getOneCategoryService } from "../../../../../src/modules/finance/financial-categories/services/getOneCategory.service.js";
...
describe("get one category integration test", () => {
  (it("should return the category for the user", async () => {
    vi.mocked(getOneCategoryService).mockResolvedValue(mockCategory);
    ...
    it("should return 400 when id is not an uuid", async () => {
      const response = await request(app).get("/finance/categories/123");
