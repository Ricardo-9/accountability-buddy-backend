...
describe("Delete goal service test", () => {
    ...
    it("should delete goal and refund initial + deposited amount", async () => {
        mockTx.financialGoal.findUnique.mockResolvedValue({ initialAmount: new Prisma.Decimal(100) })
        mockTx.goalProgressSnapshot.findFirst.mockResolvedValue({ totalDeposited: new Prisma.Decimal(1000) })
    ...
    it("should throw NOT_FOUND when goal does not exist", async () => {
        ...
        await expect(deleteGoalService(goalId, userId)).rejects.toThrow("Financial goal not found")
    })
})