...
describe("Get financial goals service test", () => {
  ...
  it("should filter by category when provided", async () => {
    vi.mocked(prisma.financialCategory.findFirst).mockResolvedValue({
      id: "c478795a-9215-4c25-9e7b-eefdc242b429",
    } as any);
  ...
  it("should throw if category is not valid", async () => {
    ...
    await expect(getGoalsService(userId, "invalid-category")).rejects.toThrow(
      "Category not found",
    );
  });
  ...
  it("should apply cursor when provided", async () => {
    await getGoalsService(
      userId,
      null,
      12,
      "86e5d547-2523-4b77-a31e-7220d68d62a4",
    );
