...
describe("Execute recurring transaction service test", () => {
  ...
  it("should execute one pending transaction", async () => {
    ...
    vi.mocked(financeAccountRepository.getAccountBalance).mockResolvedValue({
      balance: new Prisma.Decimal(1000),
    });
    vi.mocked(calculateNextOccurrence).mockReturnValue(
    ...
    expect(
      recurringTransactionRepository.updateNextOccurrence,
    ).toHaveBeenCalledWith(expect.anything(), "transactionId", {
