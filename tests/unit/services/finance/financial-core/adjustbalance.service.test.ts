...
function mockTx(overrides: {
  ...
  const updatedAccount =
    overrides.updatedAccount !== undefined
      ? overrides.updatedAccount
      : {
          id: "account-id",
          userId: "random-id",
...
describe("Adjust balance test", () => {
  ...
  it("should call update with 'increment' and create history with positive balance change", async () => {
    ...
    const result = await adjustBalanceService(
      "random-id",
      50,
      "INCREMENT",
      "INCOME",
  ...
  it("should throw an AppError (404 - NOT_FOUND) if the account does not exist", async () => {
    ...
    await expect(
      adjustBalanceService("random-id", 50, "DECREMENT", "EXPENSE"),
  ...
  it("should throw an AppError (422 - INSUFFICIENT_FUNDS) if balance < amount in 'DECREMENT'", async () => {
    ...
    await expect(
      adjustBalanceService("random-id", 200, "DECREMENT", "EXPENSE"),
    ).rejects.toMatchObject({
  ...
  it("should allow decrement when balance equals to amount exactly", async () => {
    ...
    await expect(
      adjustBalanceService("random-id", 100, "DECREMENT", "EXPENSE"),
